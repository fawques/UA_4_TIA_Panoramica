...
/**
 * It gets a 2D image from a set of 3D points. No internal order from sensor is used. 
 * It is suppose that each 3D point has an RGB value associated. The function writes 
 * extrinsic parameters to the standard output. Those parameters can be used for mapping 
 * the generated image onto the 3D data following the opposite procedure in FromImageTo3D
 * function.<br />
 * The screen data must be a set of 3D points.<br />
 */
public class From3DTo2D extends Function3D {
	...
	public void proccessData(ScreenData scr_data) throws JIPException {
		...
		for(count=0;count<elements.length;count++)
		{
			progress += prog_inc;
			element = (Point3D)elements[count];
			if(element.getDistance(0, 0, 0)>minimum)
			{
		...
		for(count=0;count<elements.length;count++)
		{
			progress += prog_inc;
			element = (Point3D)elements[count];
			origin_dist = element.getDistance(0, 0, 0);
			if(origin_dist>minimum && origin_dist<dist_image[-Y[count]+yoffset][X[count]-xoffset])
			{
				image.setRGB(X[count]-xoffset, -Y[count]+yoffset, ColorTools.convertRGBToInt(element.getColor()));
