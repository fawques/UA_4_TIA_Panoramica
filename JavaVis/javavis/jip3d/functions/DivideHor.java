...
/**
 * It divides a set of points in two subsets. To do that, it uses a horizontal plane parallel to
 * XZ plane. It also allows an interactive execution based on a manual or automatic parameter.
 * In manual case, you use a text dialog to select the division plane position in the Z axis.
 * In automatic case, you must specify a parameter which indicates the position of the division
 * plane in the Y axis.<br />
 * The screen data must be a set of 3D points.<br />
 */
public class DivideHor extends Function3D {
	...
	public DivideHor()
	{
		...
		p1.setValue(true);
		ParamFloat p2 = new ParamFloat("Height");
		p2.setValue(0.0f);
	...
	public void proccessData(ScreenData scr_data)  throws JIPException{
		...
		height = getParamValueFloat("Height");

		if(manual)
		{
			height = scr_data.getMinRange()[1];
	...
	class DivideDialog extends JDialog implements ActionListener, ChangeListener
	{
		...
		public DivideDialog(ScreenData scr_data)
		{
			...
			slider = new JSlider(JSlider.HORIZONTAL, (int)(scr_data.getMinRange()[1]*100), (int)(scr_data.getMaxRange()[1]*100), (int)(height*100));
