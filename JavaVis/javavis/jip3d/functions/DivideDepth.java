 ...
 * XY plane. It also allows an interactive execution based on a manual or automatic parameter.
 ...
 * plane in the Z axis.<br />
 * The screen data must be a set of 3D points.<br />
 */
public class DivideDepth extends Function3D {
	...
	public DivideDepth() {
		...
		p1.setValue(true);
		ParamFloat p2 = new ParamFloat("Depth");
		p2.setValue(0.0f);
	...
	public void proccessData(ScreenData scr_data) throws JIPException {
		...
		depth = getParamValueFloat("Depth");

		if(manual)
		{
			depth = scr_data.getMinRange()[2];
	...
	class DivideDialog extends JDialog implements ActionListener, ChangeListener
	{
		...
		public DivideDialog(ScreenData scr_data)
		{
			...
			slider = new JSlider(JSlider.HORIZONTAL, (int)(scr_data.getMinRange()[2]*100), (int)(scr_data.getMaxRange()[2]*100), (int)(depth*100));
		...
		/**
		 * It indicates if the state is changed, and update the new values.
